...
| Research Cursor 3 Agents/Editor product design **with Firecrawl first** | `docs/research/SOURCES.md` official Cursor docs/blog/changelog; attributed screenshot visually inspected | Product research completed, **Firecrawl search did not succeed**: a subsequent actual keyless cloud request returned HTTP403 requiring an API key for this IP (`firecrawl-attempt.json`). Official self-host search is possible in source, but no lightweight deployment was verified under current WSL capacity. Official-source fallback disclosed |
...
1. **Successful Firecrawl research remains unmet.** One real keyless search request was attempted and returned HTTP403 requiring an API key for this IP; see `docs/research/firecrawl-attempt.json`. Official self-host feasibility was researched against source, but its conservative five-service deployment was not started with only ~1.8 GiB WSL memory available and nearly exhausted swap. A usable Firecrawl API key/endpoint, or adequate resources and deployment authorization, is needed. Research content used official sources instead. No fabricated Firecrawl result.
